...
pub fn run() -> Result<()> {
    eprintln!(
        "{}",
        "🔄 Migrating secrets to fast encryption format...".cyan()
    );
    eprintln!();
    ...
    // Get all secrets
    let mut stmt =
        conn.prepare("SELECT id, project, environment, key, encrypted_value FROM secrets")?;
    ...
    eprintln!();
    eprintln!(
        "{} Migrated {} secrets to fast format",
        "✅".green(),
